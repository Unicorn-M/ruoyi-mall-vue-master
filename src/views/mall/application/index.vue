<template>
  <div class="app-container">
    ...
    <!-- 审核对话框 -->
    <el-dialog :visible.sync="reviewDialogVisible" title="贫困户申请" width="400px">
...
import {
  ...
  delUmsApplication,
  addUmsApplication,
  updateUmsApplication,
  exportUmsApplication,
} from "@/api/mall/application";
...
export default {
  ...
  methods: {
    ...
    submitReview() {
      const { id, status, explanation } = this.reviewForm; // 仅提取需要提交的字段
      updateUmsApplication({ id, status, explanation }).then(() => {
        this.$message.success("审核成功");
        this.reviewDialogVisible = false;
        this.fetchData(); // 提交成功后刷新数据
      }).catch(() => {
