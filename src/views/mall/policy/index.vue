    ...
    <div>
        <el-card>
            ...
            <el-table :data="dataList" border style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column label="政策内容" width="150">
...
<script>
import { listUmsPolicy, getUmsPolicy, addUmsPolicy, updateUmsPolicy, delUmsPolicy, getUmsInformationByPage } from '@/api/mall/policy';
...
export default {
    ...
    methods: {
        ...
        fetchData() {
            ...
            listUmsPolicy({ keyword }, { page: pageNum - 1, size: pageSize }).then(response => {
        ...
        handleEdit(row) {
            getUmsPolicy(row.id).then(response => {
                this.form = response;
        ...
        submitForm() {
            const api = this.form.id ? updateUmsPolicy : addUmsPolicy;
            api(this.form).then(() => {
        ...
        handleDelete() {
            delUmsPolicy(this.deleteRow.id).then(() => {
        ...
        handleBatchDelete() {
            delUmsPolicy(this.selectedIds).then(() => {
