...
<script>
import { listAdmin, getAdmin, delAdmin, addAdmin, updateAdmin, exportAdmin } from "@/api/mall/admin";
export default {
  components: {
  ...
  methods: {
      ...
      fetchData() {
          ...
          listAdmin({ keyword }, { page: pageNum - 1, size: pageSize }).then(response => {
      ...
      handleEdit(row) {
          getAdmin(row.userId).then(response => {
              this.form = response;
      ...
      submitForm() {
          this.$refs.form.validate(valid => {
              ...
              const api = this.form.id ? updateAdmin : addAdmin;
              api(this.form)
                  .then(() => {
      ...
      handleDelete() {
          delAdmin(this.deleteRow.userId).then(() => {
      ...
      handleBatchDelete() {
          delAdmin(this.selectedIds).then(() => {
