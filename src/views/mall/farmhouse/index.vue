  ...
  <div>
    <el-card>
      ...
      <el-table :data="dataList" border style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="农家乐内容" width="150">
          <template #default="scope">
    ...
    <el-dialog :title="dialogTitle" :visible.sync="dialogVisible" width="50%">
      <el-form ref="form" :model="form" label-width="100px">
        ...
        <el-form-item label="农家乐标题" prop="title">
          <el-input v-model="form.title" placeholder="请输入农家乐标题" />
        </el-form-item>
        <el-form-item label="农家乐内容" prop="content">
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      listUmsFarmhouse({ keyword }, { page: pageNum - 1, size: pageSize }).then(response => {
    ...
    handleEdit(row) {
      getUmsFarmhouse(row.id).then(response => {
        this.form = response;
    ...
    submitForm() {
      const api = this.form.id ? updateUmsFarmhouse : addUmsFarmhouse;
      api(this.form).then(() => {
    ...
    handleDelete() {
      delUmsFarmhouse(this.deleteRow.id).then(() => {
    ...
    handleBatchDelete() {
      delUmsFarmhouse(this.selectedIds).then(() => {
