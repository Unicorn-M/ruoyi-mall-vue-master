<template>
    <div>
      <el-card>
        ...
        <div class="search-bar">
          <el-input v-model="queryParams.title" placeholder="请输入标题查询" clearable style="width: 300px;" />
  ...
  <script>
  import { listUmsNotice, getUmsNotice, delUmsNotice, addUmsNotice, updateUmsNotice } from "@/api/mall/notice";
  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        listUmsNotice(query, pageReq).then((response) => {
      ...
      handleUpdate(row) {
        getUmsNotice(row.id).then((response) => {
          this.form = response;
          this.open = true;
          this.title = "编辑公告";
        });
      ...
      handleSingleDelete() {
        delUmsNotice(this.deleteRow.id).then(() => {
      ...
      handleBatchDelete() {
        delUmsNotice(this.selectedIds).then(() => {
      ...
      submitForm() {
        if (this.form.id) {
          updateUmsNotice(this.form).then(() => {
            this.open = false;
            this.getList();
          });
        } else {
          addUmsNotice(this.form).then(() => {
            this.open = false;
            this.getList();
          });
        }
